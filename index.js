...
async function run() {
  try {
    ...
    const announcementCollection = client.db('manageNest').collection('announcements');
    const cuponCollection = client.db('manageNest').collection('cupons');
    const paymentCollection = client.db('manageNest').collection('payments');
    ...
    app.get('/bookingApartments/:id', async (req, res) => {
      const id = req.params.id;
      const objectId = new ObjectId(id);
      const result = await bookedApparmentCollection.findOne({ _id: objectId });
      res.status(200).json(result);
    });
    app.get('/acceptedApartments/:email', async (req, res) => {
      const email = req.params.email;
    ...
    app.post('/paymetpovided', async (req, res) => {
